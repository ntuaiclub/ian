...
**國立臺灣大學人工智慧應用社 (NTU AI Club)** 的多平台智慧客服系統，整合 Discord、Facebook Messenger 及 LINE，提供 24/7 自動化問答服務。
...
| `MCP_HOST` | MCP Server 綁定位址（預設 `0.0.0.0`） |
| `MCP_PORT` | MCP Server 連接埠（預設 `5191`） |
| `NGROK_AUTHTOKEN` | ngrok Auth Token |

### Docker Compose（推薦）

需要 NVIDIA GPU 及 [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html)。

```bash
docker-compose up -d --build   # 建置並啟動（含 ngrok tunnel）
